...
## 当前性能对比(2026-09-18 · 第 1 轮修复后 · §623)

### 引擎级(同形状、同真实 V4.1 第 3 层权重、THREADS=60、同一个 lk 二进制)
| 形状 | 修前 | **修后** | lk_moe | 我们/lk 修前 → 修后 | 达标线(≤90% ⇒ 比值 ≤1.111) |
|---|---|---|---|---|---|
| BS=1 DEDUP=6(解码) | 0.59 | **0.54** | 0.44 | 1.34× → **1.23×** | 0.489 ms ❌ |
| BS=227 DEDUP=23 | 51.03 | **40.65** | 29.59 | 1.70× → **1.37×** | 32.88 ms ❌ |
| BS=1893 DEDUP=23 | 395.78 | **310.89** | 229.19 | 1.78× → **1.36×** | 254.7 ms ❌ |

### 门禁形状(`check_engine_aligned.sh`,BS=6,同日自带 lk 对照)
| 形状 | 我们 | lk_moe | 比值 | 达标 |
|---|---|---|---|---|
| DEDUP=12 | **0.56 ms/层 · 270 GB/s · 2.25 GB/s·线程** | 0.54 | **1.04×** | ✅ |
| DEDUP=23 | **0.69 ms/层 · 365 GB/s · 3.04 GB/s·线程** | 0.65 | **1.06×** | ✅ |

**本轮结论**:差距从 **1.34-1.78×** 收到 **1.23-1.37×**;门禁口径已 **1.04-1.06×**(追平)。
数值门 `OK=7 BAD=1` / `max_rel=1.873e-02` **逐位未变**。
**注意两种口径的差别本身是信息**:门禁是小 batch(BS=6)、A/B 是大 batch + 低去重比
(DEDUP=23、BS=227/1893)⇒ **我们的劣势随"活跃专家多、每专家 M 小"而放大**(与 §617 同源)。

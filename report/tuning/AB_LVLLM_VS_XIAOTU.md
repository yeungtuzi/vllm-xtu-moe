...
## 当前性能对比(2026-09-18 · 第 3 轮:§623 + §625 + **§628 线程数突破**)

### ⚠️ 先纠正形状:真实负载是 `na≈226`,不是 `DEDUP=23`
§619 的服务实测是 `[NS-PROF] na=226.2`。此前引擎级调优用 `DEDUP=23` **不够真实**:
| 形状 | DEDUP=23 | **DEDUP=226(真实)** |
|---|---|---|
| BS=227 | 1.18× | **1.27×** |
| BS=1893 | 1.21× | **1.28×** |

### 引擎级(真实形状 DEDUP=226、同真实 V4.1 第 3 层权重、同一个 lk 二进制)
| 形状 | THREADS | xiaotu | lk_moe | 我们/lk | 达标线(≤1.111) |
|---|---|---|---|---|---|
| BS=227 | 60 | 38.63 | 30.40 | 1.27× | 33.78 ❌ |
| BS=1893 | 60 | 279.33 | 218.95 | 1.28× | 243.3 ❌ |
| **BS=1893** | **180** | **123.28** | **112.23** | **1.10×** | **✅ 达标** |
| BS=1893 | 192 | **114.58** | 191.40(lk 崩) | 0.60× | ✅ |
| 解码 BS=1 DEDUP=6 | 60 | 0.54 | 0.44 | 1.23× | 0.489 ❌ |

**线程数扫描(BS=1893 / DEDUP=226,单引擎)** —— 两边都远没吃满,`THREADS=60` 是**在错误形状下调出来的**:
| THREADS | 30 | 60 | 90 | 120 | 150 | 180 | 192 |
|---|---|---|---|---|---|---|---|
| xiaotu ms/层 | 535.58 | 283.99 | 182.82 | 145.39 | 132.89 | **123.28** | **114.58** |
| lk ms/层 | 490.35 | 218.22 | 157.39 | 129.02 | 121.16 | 112.23 | 191.40 |
| 我们/lk | 1.09× | **1.30×** | 1.16× | 1.13× | **1.10×** | **1.10× ✅** | 0.60× |
| 相对 60 线程 | 0.53× | 1.00× | 1.55× | 1.95× | 2.14× | **2.30×** | 2.48× |

### 门禁形状(BS=6,同日自带 lk 对照)
| 形状 | 我们 | lk_moe | 比值 | 达标 |
|---|---|---|---|---|
| DEDUP=12 | **0.56 ms/层 · 270 GB/s** | 0.54 | **1.04×** | ✅ |
| DEDUP=23 | **0.68 ms/层 · 370 GB/s** | 0.61 | **1.11×** | ✅ 边界 |

### 服务级(同 env / 等效参数 / 逐字节相同 prompt / 纯度断言;已清 `/dev/shm`)
**本轮正在重测 `THREADS=96`(两 arm 同值)**,上一轮(THREADS=60):
| 格子 | arm A(lk_moe) | arm B §625 | B/A |
|---|---|---|---|
| 256 / 32 | 11.19 | 8.22 | 0.73× |
| 256 / 1024 | 40.08 | 28.24 | 0.70× |
| 8192 / 32 | 0.49 | 0.40 | 0.82× |
| 8192 / 1024 | 11.70 | 9.08 | 0.78× |

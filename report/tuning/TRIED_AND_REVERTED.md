...
## R109. `XIAOTU_MOE_GEMM_NR=16`(M==1 专用 GEMV,16 行在途提 MLP)—— 反而慢
- **动机**:本内核每核 ~1.2 GB/s vs 单核纯流式 36 GB/s ⇒ 怀疑 MLP 不足;通用路径 NR=16 会因
  `acc[MR][16]`(MR=4 ⇒ 64 zmm)溢出,故为 M==1 单独写 NR=16 路径(只需 16 个累加器)。
- **结果**(harness,96 线程,真实一层):`compute` **0.36 → 0.45 ms**(慢 25%)。
- **结论**:加大在途行数**不能**提升本内核的带宽 ⇒ 瓶颈不是 MLP,也不是指令数(PERMV 只换来 10%);
  当前 0.35-0.36 ms/层(76 MB,211 GB/s)已是这套代码结构 + 4 核/CCD 下的稳态。已回退。

## R110. `XIAOTU_MOE_NCGU`(并行区的 N 分块数 4/8/16/32)—— 无收益
- **动机**:§329 定位到"每层并行区固定成本"后,猜测 job 数越多 → 票务/唤醒开销越大,
  故试把每专家的 N-chunk 数从默认(≈`nt*2/na`≈32)降到 4/8/16。
- **结果**(harness,NENGINES=14/REP=150/QLEN=1/DEDUP=6/96 线程):
  `NCGU=4/8/16/32` → **336.5 / 333.6 / 336.0 / 336.0 µs/层** ⇒ 全在噪声内。
- **结论**:该路径的固定成本**不是由 job 数决定的**(票务本身很便宜);
  §329 里真正的 63 µs 是**发布前的 `pause` 自旋**。旋钮保留(默认不设),不作为调优手段。

## R111. `XIAOTU_MOE_DIAG_BARRIER` 的推论:phase B(down)没有 ablation 开关 —— **诊断能力缺口,非回退**
- **现象**:`DIAG_BARRIER=1` 只把 **phase A**(gate/up)换成空转,phase B(down)照常计算。
  因此 `A` 能拆成"并行区成本 0.0975 + 真实计算 0.045",而 `B`(0.135 ms)**无法同法拆分**。
- **动作**:不引入新的诊断开关(避免继续膨胀代码);下轮若要继续压每层成本,
  应直接读 `numa_pool` 的 `entered_/left_/issued_/abandoned_` 计数 + 用 `perf` 采样式定位。

## R112. `XIAOTU_MOE_NSHARD`(1/2/4/8)用于小批量 —— 默认 8 已是最优,无收益
- **动机**:§330 定位到"每层两个并行区各 ~0.06 ms 固定成本"后,猜测**减少分片节点数**能让
  发布/收敛更便宜(NSHARD=1 时代数发布只写 1 份 node_nj_/node_base_ 而不是 8 份)。
- **结果**(harness,NENGINES=14/REP=150/QLEN=1/DEDUP=6/LK_THREADS=120/SETTLE=200):

...
**结论(直接回答用户的问题)**:
* 单张 40GB **能**跑投机,但 `gpu_util` 的账里没有 lk 引擎缓冲(~5.3 GiB)+
  图缓冲(~1.5-2 GiB)+ 预热临时显存(~2 GiB),所以**单卡 + 草稿常驻(10.12 GiB)+ 图**
  在 0.87/0.90 都 OOM;**要跑就得降到 util ≈0.80-0.82**,KV 只剩 ~4 GiB(≈1.1 万 token)。

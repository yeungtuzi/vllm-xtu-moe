...
**状态更新(§565,2026-09-17):第 1 条已"默认开启"并通过正确性闭环。**
* 第 2 条(**大权重逐层 load/slice/release**)仍**待做** —— 目前 `RELEASE_SOURCE` 只在
  "全部物化之后"释放,峰值仍含所有源张量(650 GiB vs 必要 ~467 GiB)。
* 第 1 条的落地形态:`XIAOTU_ENGRAM_LAST` 默认 **1**。§562d 的"greedy 跨臂 1/5"已查明是
  **我们的 shard 偏移 bug**(§563),修复后 greedy **5/5 逐字节相同**;并新增**表内容级校验**
  (§564,默认开,fail-closed,实测 8/8 PASS)。
* 新增约束:`--load-format dummy` 时**不读真表**(保留占位填充),否则纯属浪费 189 GiB 磁盘读(§565)。
...
**修正(§586,2026-09-18):优先级 4「专家层常驻(收益最高)」的"收益最高"前提已被实测推翻。**
* 服务级(TP=2/8K/5 层常驻):TPOT 43.19 vs 无常驻 42.86 ms(**+0.33 ms,中性偏负**);
  C=4 聚合 41.42 vs **46.92 t/s(−12%)**;
* 根因:GPU 小 M MoE 比 CPU 引擎**慢 3-5×**(M=1:1.272 vs 0.38 ms/层,随 M 线性)⇒
  常驻等于把"0.38 ms 的 CPU 计算"换成"1.27 ms 的 GPU 计算";
* ⇒ **新的默认:优先级 4 放 0 层**,这块显存**改投 KV**(每层 3.36 GiB/rank ≈ 1.5M token;
  8K 档共省 16.8 GiB ≈ **+9.1M token KV**),而延迟不变或更好;
* **复查条件**:只有当小 M GPU MoE 被优化到 ≥ CPU 引擎(0.38 ms/层)时,常驻才值得重新开启
  —— 它仍是唯一能同时删掉"引擎时间 + 往返"的手段。恢复旧行为:`XIAOTU_RESIDENT_POLICY=1`。
...
## R14/SPIN(2026-09-17). **线程池自旋必须是"几百微秒",绝不许设 0** —— 一个被过度推广的结论害了我们一整个 session
* **事实链**:§355 记录过 `SPIN_IDLE_US=5000` 是正反馈灾难(48 层 × 3 相 × 5 ms ⇒ 池几乎永不停转)
  ⇒ 当时把启动器默认**钉成 0**。但那是**过度推广**:0 意味着每个 phase 的唤醒都走 futex,
  **每层白付 ~0.5 ms**。实测(两次独立,§526 与 §557):
  | `XIAOTU_MOE_SPIN_IDLE_US` | 解码形状 ms/层 |
  |---|---|
  | **0** | **0.94 - 1.00** |
  | 300(插件 setdefault) | **0.44** |
  | 1000 / 未设 | 0.44 / 0.43 |
* **服务级代价**(同旗标只改 SPIN):C=1 聚合 18.62 → **26.53 t/s**、**TPOT 48.32 → 33.41 ms(−31%)**、
  每层 engine 0.67 → 0.36 ms;固定 16-prompt 集 TPOT 49.84 → 40.68 ms。
* ⇒ **规则**:`SPIN_IDLE_US` 的正确区间是 **100-1000 µs**;启动器默认 **300**(= 插件默认)。
  任何"某旋钮取极大值有害 ⇒ 那就取 0"的推理都**必须再测 0 这一点**(单调性不成立)。
* 已改:`serve_v41.sh`、`xtu_own_v41_mem.sh` 默认 300,注释同步更正。

## R15(2026-09-18). **读数前先对齐尺子**:同名指标在不同"调用预算/形状/路径"下不可比 —— 三次事故的共同根因
本轮连撞三次"尺子不一致导致的假结论",全部靠**先对齐再读数**纠正:

1. **冷启动**:我们的引擎前 ~50 次调用才到稳态(`WARMUP=1→1.01`、`50→0.70`、`200→0.70` ms/层),
   而 lk 与调用次数无关(恒 ~0.58)⇒ 性能门禁不加 `WARMUP` 会报**假 FAIL(1.62×)**,同尺子是 1.19×。
   ⇒ `bench_engine_ab.py` 加 `WARMUP`(验收 200),门禁两臂都传。
2. **形状**:阈值只在 `DEDUP=12` 上标定却套到 `DEDUP=23`(历史本就是 1.24-1.27)⇒ 门禁永远红。
   ⇒ 阈值**按形状分别标定**,基线写进脚本注释。
3. **长度**:`prompt_logprobs` 在短 prompt 上逐位可复现、长 prompt 上不可(0.12~0.38 nats,§572)⇒
   用逐位 logprob 当长上下文门槛 = 永远红。⇒ 长上下文用"首 token/贪心文本一致"。

**规则**:报数前先写清"**同一进程/同一形状/同一调用预算/同一路径**",否则数字只能当趋势,不能当验收。

## R16(2026-09-18). **并发协议的发布顺序必须"全局可见先于观察点"** —— 只用 release store 不够
x86 TSO 下 `release` store 进 store buffer,紧随其后的 load **可以先执行** ⇒ 观察者会拿到
"自洽但陈旧"的快照(seqlock 复读检不出:它只看奇偶,不做跨变量陈旧检测)。
* 反面教材:线程池丢票竞态(§569)——worker 领到本代的票却按旧快照判越界 ⇒ 票被丢弃 ⇒ `remaining_` 永挂。
* **正确做法**:发布方在"读快照"之前用 **`seq_cst` store(或显式 fence)** 保证发布值已全局可见。
* **推论**:凡是"先发布、后读某个计数器当基线"的协议,都要按这条检查一遍。

## R17(2026-09-18). **"照搬上游机制"之前先查它的前提条件** —— 属性、类型、缓存规格三处都可能不匹配
③ CED 一轮里连撞三次:
1. `kv_sharing_fast_prefill` 的 eligible 用的是 `Attention` 类型,而 V4.1 的类是 `AttentionLayerBase`
   ⇒ **结构上必然空集**(不是"没配置");
2. 该机制的 `get_kv_cache_spec()` 遇到 `kv_sharing_target_layer_name` 就 `continue`
   ⇒ 用那个属性登记会**丢掉本层自己的 SWA 缓存**(V4.1 每层必须有自己的 SWA);
3. 该机制只改 **attention metadata**,而本机预填充由 **CPU MoE 主导** ⇒ 不切隐藏状态几乎无收益。
**规则**:复用上游机制的判据是"**前提条件逐条对上**",不是"名字看起来一样"。

## R18(2026-09-18). **长上下文测量必须声明"跑的哪条路径"**(CPU 引擎 vs GPU 预填充)
* CPU 引擎**逐位确定**(11 次 10/10);GPU 预填充路径**不逐位确定**(微基准:同输入三次重跑,
  2.85% 元素不同、相对 ~0.4%,原子规约特征)⇒ 服务级长 prompt 日志概率天然有 0.1~0.4 抖动。
* 凡是"跨运行比较输出"的实验,必须写清 `VLLM_XIAOTU_GPU_PREFILL_MIN_TOKENS` 取值,否则结论不可比。

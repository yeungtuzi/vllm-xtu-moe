...
SPIN="${SPIN:-5000}"   # serve_v41.sh 的默认;0 = 完全不自旋(serve_mainline.sh 的默认,§355 证明 5000 是正反馈灾难)
RANK_SPLIT="${RANK_SPLIT:-1}"   # 1=按 node 子集切(默认;NPS1+TP2 会退化成 2×socket 副本) 2=CCD 交错(每 rank 覆盖全部 node ⇒ nshard=node 数 ⇒ 1 份权重)
...
XV="$ROOT/report/tuning/logs/$TAG.envfile"
{
  ...
  echo "XIAOTU_GPUPREFILL_WCOPY=$WCOPY"
...
XIAOTU_MOE_SPIN_IDLE_US=$SPIN XIAOTU_MOE_GPU_RESIDENT_LAYERS= \
XIAOTU_MOE_RESIDENT_BUDGET_GB=0 XIAOTU_RELEASE_SOURCE=$RELEASE_SOURCE \
XIAOTU_GPUPREFILL_WCOPY=$WCOPY XIAOTU_MOE_RANK_SPLIT=$RANK_SPLIT \

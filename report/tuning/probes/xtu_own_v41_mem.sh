...
CED="${CED:-0}"            # 1 = XIAOTU_CED_FASTPREFILL=1(③ CED 预填充捷径,需同时 KVSHARE=1)
...
XV="$ROOT/report/tuning/logs/$TAG.envfile"
{
  ...
  [ "$CED" = "1" ] && echo "XIAOTU_CED_FASTPREFILL=1"
  [ "$CED" = "1" ] && echo "XIAOTU_CED_WINDOW=${CED_WINDOW:-255}"
...
${GPF_V2:+XIAOTU_GPF_V2=$GPF_V2} ${CED:+XIAOTU_CED_FASTPREFILL=$CED} ${CED:+XIAOTU_CED_WINDOW=${CED_WINDOW:-255}} ${GPF_STAGE:+XIAOTU_GPF_STAGE=$GPF_STAGE} ${GP_SPLIT:+XIAOTU_GP_SPLIT=$GP_SPLIT} \

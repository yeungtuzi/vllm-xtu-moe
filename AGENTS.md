...
## ⛔⛔ 第 0 条之前(最高纪律;用户 2026-09-24 明令)

**凡是面向"别人项目"的 GitHub 写操作,必须经用户逐个手工批准审阅后才能执行** ✓ ——
未获**逐条**批准,一律**不得**发出:

* ✗ `gh pr create` / `gh pr edit` / `gh pr ready`(取消 Draft)/ `gh pr comment` / `gh pr review`
* ✗ `gh issue create` / `gh issue comment` / `gh issue close`
* ✗ 任何 `gh api -X POST/PATCH/PUT/DELETE` 指向他人仓库
* ✗ **push / force-push 到 fork 里承载 PR 的分支**(这会公开更新他人的 PR 页面 ✓)
* ✗ 任何形式的评论、回复、贴标签、@ 提及

**流程** ✓:先把**准备发出的原文**给用户看 ⇒ 用户**明确同意这一条** ⇒ 才执行 ✓;
**一条一批**,不得用"上次同意过类似的"类推 ✓。
**读取类**操作(GET / search / 拉取)不受限 ✓;**我们自己仓库**的提交与推送不受限 ✓(除非用户另行指示 ✓)。
若不确定是否属于"别人项目",**先问** ✓。
...
## ⛔ 文档归属纪律(用户 2026-09-24,已纠正三次)

**`README.md` = 项目的发布与介绍面**。只放**面向用户/发布**的内容:
安装、快速开始、版本变更、实测性能、已知限制、运行手册、逐模型指南 —— 以及这些文档的索引 ✓
* ❌ **不要把"远景规划 / 路线图 / 内部设计 / 方案报告"挂进 README** ✗
  (本会话已因此被纠正:`DEPLOYMENT_FLEET`、`CLUSTER_DESIGN`、`ARCHITECTURE_OVERVIEW` ✗)
* ❌ **不要把规划类文档放进公开的 `docs/`** ✗ —— `docs/` 属**发布面**;
  规划/设计/路线图一律进 **`dev-docs/`**(gitignored ✓,用户将来择机清理后再发布 ✓)
* ✅ 判据:这份文档**外部用户现在就该看**吗?是 ⇒ `docs/`(+ 可进 README 索引 ✓);
  否(是"我们打算怎么做")⇒ **`dev-docs/`** ✓
* ✅ **台账/日志是历史记录**:里面的旧路径**不改写** ✗(当时事实 ✓);位置变化只在**新增条目**里说明 ✓

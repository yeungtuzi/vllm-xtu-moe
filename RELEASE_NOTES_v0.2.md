...
### 3.6 ⚠️ 与 `lvllm + lk-moe` 的等效参数同机对比(**我们更慢,如实公布**)
...
* 分解:**引擎层面固有 1.1-1.3×**(我们自己的微基准门禁长期报"同日 lk 比值 1.17/1.23",
  2026-09-11 起点是 2.14×,在持续收敛)+ **配置/集成约 1.2-1.3×**(旋钮扫描中)。
* **结构性根因**:我们的 GEMM 沿 `K` 维向量化,lk_moe 沿**输出列**向量化;解码 M=1 时
  后者权重每字节只用一次、无需跨线程归约 ⇒ 小 M 场景天然占优。
* **本版相对上一版仍是净进步**(§3.2/§3.5:同口径 TTFT −40%,唯一 prompt 预填充 +30%),
  但**不应被读成"已追平/超过参考实现"**。

### 3.7 长 prompt(DSH 真实开发会话回放)

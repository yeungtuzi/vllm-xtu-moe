...
- **解码停顿:原因已查明**(见 EXPERIMENTS **B126–B131**)—— 不是引擎、不是 CPU 争用、不是 chunk 大小,
  而是 **GPU 预填充每层权重的 H2D 上传未与计算重叠**:实测 `dma=533 ms/层`(占 staging 98.8%,GLM FP8 ~3.4 GiB/层),
  于是任何带预填的 step 都要付 `层数 × 540 ms`(≈24 s);同一 step 里有解码时就表现为 10–14 s 的 ITL 尖峰。
  **规避**:`GPU_PREFILL_MIN=0` 关掉 GPU 预填(已验证消除尖峰)。**根治方向(0.2.5)**:减少 H2D 的**字节量/重复次数** —— ping/pong 双缓冲**早已实现且启用**,
  但流水线深度只有 1 层、每层 H2D(533 ms)远大于每层计算,故重叠只能藏掉一小部分(更正见 EXPERIMENTS **B139**)。

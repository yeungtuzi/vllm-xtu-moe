...
L2_GB="${L2_GB:-100}"
TRANSFER_MODE="${TRANSFER_MODE:-lmcache_driven}"   # lmcache_driven|engine_driven|auto                       # 磁盘容量上限
PORT="${PORT:-5555}"                        # connector 默认 tcp://localhost:5555
mkdir -p "$L2_DIR"
echo "[lmcache] L1=${L1_GB}GB L2=${L2_DIR}(${L2_GB}GB) port=${PORT} transfer=${TRANSFER_MODE}"

#!/usr/bin/env python
"""Stable E=256 timing: single warmup + single timed cpu_prefill per B,
with fresh arrays and gc between batches (matches realistic one-call-per-layer
usage; avoids the tight-loop NumaWorkPool race seen under oversubscription)."""
import glob
import json
import os
import sys
...
def main():
    ...
    print("  B | per-layer ms | 43-layer ms | ceiling tok/s", flush=True)
    for B in [1, 2, 4, 8, 16, 32, 48, 64, 96, 128, 192, 256]:
        ...
        engine.cpu_prefill(B, K, ids, wts, x, out)  # warmup
        gc.collect()
        t0 = time.perf_counter()
        engine.cpu_prefill(B, K, ids, wts, x, out)
        dt = (time.perf_counter() - t0)
        per_layer_ms = dt * 1e3
        tok_per_s = B / (per_layer_ms * N_LAYERS / 1e3)
        print(f"  {B:3d} | {per_layer_ms:8.3f} | {per_layer_ms*N_LAYERS:9.1f} | "
              f"{tok_per_s:9.1f}", flush=True)
        del x, ids, wts, out
        gc.collect()

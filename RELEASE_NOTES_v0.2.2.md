...
**本项目(发行名 `vllm-xtu-moe`)不发布到 PyPI。** 两种装法:

**(a) GitHub Release 附件**(推荐,不需要本地编译器):
...
**(b) 源码安装**:`CXX=g++-16 PYTHON=$(which python) bash scripts/build_engine_variants.sh
&& pip install -e .`(见 `docs/RUNBOOK.md` §2 方式 B)。
...
* **引擎包 `xiaotu_moe` 另有独立的 PyPI 发行名
  [`xiaotu-moe`](https://pypi.org/project/xiaotu-moe/)**(engine-only,由引擎自身源码发布,
  当前线 0.1.0/0.11/0.12)。本插件的 wheel **已把引擎一起打进去**,所以装本插件不需要它,
  也**没有**在本版对它做任何上传或 yank;
* 需要自定义内核开关或换编译器时,走源码路径。

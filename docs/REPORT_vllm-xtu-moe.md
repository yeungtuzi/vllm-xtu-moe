...
| 编号 | 指标 | 目标 | 当前 | 状态 |
|---|---|---|---|---|
| A1 | 不投机解码 **C=1 TPOT** | **≤ 30 ms**(力争 25–27;参考 23.11) | **26.48 ms** 🎯 | ✅✅ |
| A2 | 不投机解码 **C=4 聚合吞吐** | **≥ 50 t/s**(目标 60+) | **104.98 t/s**(长输出 128.15) | ✅✅ |
| A3 | 每层 `rest`(非 CPU 段) | ≤ 0.40 ms,且 `compute(engine/ep)` 不退化 | **稳态最小 0.335 ms**(均值 0.664 含预填充空档);compute **0.239 ms**(历史 0.41) | ✅/🟡 见 §6.5B |
...
|---|---|---|---|---|---|
| C=1 | 23.11 ms | 41.04 t/s | **26.48 ms** | 34.53 t/s | 0.87× |
| C=2 | 31.75 ms | 59.73 t/s | **28.81 ms** | **63.19 t/s** | **1.10×** ✅ |
| C=4 | 41.88 ms | 86.92 t/s | **31.79 ms** | **104.98 t/s** | **1.32×** ✅ |
...
| ④ | **+ 并行区发布自旋 2000→200**(本轮) | **26.48 ms** | **63.19** | **104.98** |
| | **累计** | **−29%** | **+84%** | **+206%** |
...
| 指标 | 均值 | 最小(稳态下界) |
|---|---|---|
| period(相邻两次 `cpu_decode` 间隔) | 0.929 ms | 0.574 ms |
| **compute**(CPU MoE,engine+EP) | 0.262 ms(**engine 0.239**,EP 0.023) | **0.239 ms** |
| **rest**(GPU + 拷贝 + 握手) | 0.667 ms | **0.335 ms** |
| 折算每模型层(×31/43) | 0.481 ms | **0.242 ms** |

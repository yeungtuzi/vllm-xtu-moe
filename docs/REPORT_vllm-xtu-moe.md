...
```
                   CPU 参与 MoE 计算?
                        否                是
                 ┌──────────────────┬──────────────────────┐
   开源 / 可改    │ vLLM 主线        │ ktransformers        │
                 │ (CPU=虚拟显存)   │ (重心已转向微调/     │
                 │                  │  SGLang/消费级卡;    │
                 │                  │  依赖 Intel AMX)     │
                 ├──────────────────┼──────────────────────┤
   闭源 / 不可改  │        —         │ lk_moe / Lvllmds4-x  │
                 │                  │ (Proprietary,禁衍生) │
                 └──────────────────┴──────────────────────┘
                                    ▲
                        ★ vllm-xtu-moe(本项目)
             开源可改 + CPU 参与计算 + 性能对标专有引擎
```
...
```
┌───────────────────────── vLLM fork(编排层,与参考实现逐字节同源)─────────────────────────┐
│  Scheduler / PagedAttention / CUDA Graph / Speculative(DSpark)                          │
│                                                                                          │
│   RoutedExperts.forward()                                                                │
│     ├─ is_gpu_resident_layer?  ──是──►  vLLM 原生 GPU MoE(MARLIN MXFP4)                  │
│     ├─ GPU prefill 层 + batch≥阈值? ──► _gpu_prefill(GPU)                                │
│     ├─ 预填充 ─────────────────────► _cpu_prefill(CPU 引擎)                             │
│     └─ 解码 ───────────────────────► _cpu_decode(CPU 引擎)  ◄── 本项目主战场            │
└──────────────────────────────────────────┬───────────────────────────────────────────────┘
                                           │ 7 个指针 + stream
                    ┌──────────────────────▼──────────────────────┐
                    │  xiaotu_moe(本项目自研引擎,C++/AVX-512)     │
                    │   • MXFP4 权重常驻主机内存,N 分片按 NUMA mbind│
                    │   • 每 shard 一个工作线程池(PERMV 解码)      │
                    │   • 跨 rank EP 归约走 /dev/shm(不走 NCCL)    │
                    │   • 异步握手:常驻 worker + mapped flag       │
                    └─────────────────────────────────────────────┘
```
...
> 对科研与教学来说,后者可能比前者更重要 —— 因为它让模型**可以被研究**,而不只是被调用。

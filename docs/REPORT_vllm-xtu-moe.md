...
| 编号 | 指标 | 目标 | 当前 | 状态 |
|---|---|---|---|---|
| A1 | 不投机解码 **C=1 TPOT** | **≤ 30 ms**(力争 25–27;参考 23.11) | **26.17 ms** 🎯 | ✅✅ |
| A2 | 不投机解码 **C=4 聚合吞吐** | **≥ 50 t/s**(目标 60+) | **107.34 t/s**(长输出 128.15) | ✅✅ |
| A3 | 每层 `rest`(非 CPU 段) | ≤ 0.40 ms,且 `compute(engine/ep)` 不退化 | **稳态最小 0.339 ms**(均值 0.666 含预填充空档);compute **0.195 ms**(历史 0.41) | ✅/🟡 见 §6.5B |
...
5. **把"每层固定成本"从带宽里剥出来,并定位到一个旋钮**
   * 先用 `DEDUP` 扫描(固定 FLOPs、只变字节)证明**边际带宽已到机器上限(692–768 GB/s)**,
     从而排除"带宽墙";再用引擎自带分段计时 + `DIAG_BARRIER` 空转把每层拆成
     "真实计算"与"并行区固定成本";最后在代码里找到那个固定成本 =
     **每个并行区发布前的 2000 次 `pause` 有界自旋**(Zen4 上 ≈63 µs/区)。
   * 改 `PUBLISH_SETTLE 2000 → 200`(保留 91% 的收益 + 保留保护),端到端
     **C=1 28.13 → 26.48 ms、C=4 聚合 100.2 → 105.0 t/s**,数值门禁逐字不变。
6. **"用实测分解代替猜测"的诊断体系**
...
|---|---|---|---|---|---|
| C=1 | 23.11 ms | 41.04 t/s | **26.17 ms** | 35.03 t/s | 0.88× |
| C=2 | 31.75 ms | 59.73 t/s | **28.41 ms** | **64.27 t/s** | **1.13×** ✅ |
| C=4 | 41.88 ms | 86.92 t/s | **31.27 ms** | **107.34 t/s** | **1.37×** ✅ |
...
| ③ | + 12 层常驻 + 异步 flag 握手 | 28.13 ms | 59.89 | 100.16 |
| ④ | + 并行区发布自旋 2000→200 | 26.48 ms | 63.19 | 104.98 |
| ⑤ | **+ 每 CCD 5 核(THREADS 48→60)**(本轮) | **26.17 ms** | **64.27** | **107.34** |
| | **累计** | **−30%** | **+87%** | **+213%** |
...
| 指标 | 均值 | 最小(稳态下界) |
|---|---|---|
| period(相邻两次 `cpu_decode` 间隔) | 0.916 ms | 0.534 ms |
| **compute**(CPU MoE,engine+EP) | 0.250 ms(**engine 0.235**,EP 0.015) | **0.195 ms** |
| **rest**(GPU + 拷贝 + 握手) | 0.666 ms | **0.339 ms** |
| 折算每模型层(×31/43) | 0.480 ms | **0.244 ms** |

...
| **P1** | 在插件里接 `fused_experts` + 一个 **1 层 staging**,先只对**单层**验证数值 | 与 CPU 路径的预填充输出**逐元素对齐**(rel < 1e-2) | 1 天 |
| **P2** | 全层接入 + 阈值 `T` 标定 + 显存预算 | `bench_lat.sh` L=512/2048/8192 的 **TTFT** 与 fork 对照 | 1 天 |

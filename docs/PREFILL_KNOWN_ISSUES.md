...
## 5b. ⚠️ 交付阶段的硬约束：**MiMo 在 256K 下不可行**

新目标要求在 **256K 上下文**下测三个模型的 long prefill。**按引擎反算的每 token KV 核算，MiMo 做不到：**

| 模型 | B/token（引擎反算） | 256K 需要 | 单卡可用(util 0.90) | 可行? |
|---|---|---|---|---|
| GLM-5.3-Flash | 19,505 | **4.8 GiB** | 35.5 GiB | ✅ |
| **MiMo-V2.5** | **253,633** | **61.9 GiB** | 35.5 GiB | ❌ **差 1.74×** |
| DeepSeek-V4.1 | 30,639 | **7.5 GiB** | 35.5 GiB | ✅ |

**MiMo 的可达上限**（扣掉权重+激活约 12 GiB 后 KV 23.5 GiB）：**约 100K token**；
即使把全部显存给 KV 也只有 150K。**⇒ 256K 超过其单卡能力 1.74 倍。**

**这不是配置问题，是模型架构决定的**（MiMo 的 KV 是 bf16、每 token 约 248 KiB，
而 GLM 只有 19 KiB —— 相差 13 倍）。

**⇒ 交付阶段必须二选一（需用户定）：**
1. **MiMo 用它的可达上限（约 100K 或 128K）**，在表里注明「256K 不可行，此为单卡上限」；
2. 或 **MiMo 不参与 256K 这一轮**，只在低上下文档给出成绩。

**在得到答复前，我会先跑 GLM 与 V4.1 的 256K**（两者都 ✅），MiMo 按第 1 条跑其可达上限并明确标注。

...
# 【第 219 轮·续】每层固定成本的进一步下探:`PUBLISH_SETTLE` 2000 → 200(端到端 −5.9%)

> 承上章"七、下一轮的唯一目标:每层 ~0.23 ms 的非字节成本"。本章给出**定位过程与落地结果**。

## 一、定位链(全部在分钟级 harness 完成)

1. **先排除带宽**:`DEDUP` 扫描(固定 FLOPs、只变字节)⇒ 边际带宽 **692–768 GB/s**,
   已在机器上限(~740)的 93–100%。**"让每字节搬得更快"这条线到顶了。**
2. **再用引擎自带分段计时** `XIAOTU_MOE_PROFILE=1`(注意 `prof_every_=40` 是**每引擎**计数):
   ```
   [MOE-PROF] calls=40 na=6 M=1 A=6.7ms A2=0.3ms B=5.4ms C=0.4ms ovh=0.0ms (sum 13ms)
   ```
   ⇒ A(gate/up + 融合 SiLU)= 0.168 ms/层,B(down)= 0.135 ms/层。
3. **用 `XIAOTU_MOE_DIAG_BARRIER=1` 把"计算"与"并行区本身"分开**:
   A 从 0.168 → **0.0975 ms** ⇒ phase A 有一半是**并行区固定成本**。
4. **回到代码**找到 `numa_pool.hpp` 发布前的
   `for (i < settle_iters) __builtin_ia32_pause();`,默认 **2000**;
   Zen4 的 `pause` 不是空操作(数十周期)⇒ ≈ **63 µs/区**。
5. **A/B(各跑两遍)**:2000 → 394.9/397.2 µs/层;200 → **332.1/334.0**(−16.0%);
   0 → 326.6/325.4(−17.6%)⇒ **200 拿到 91% 收益且保留保护**。

## 二、落地结果(服务端,同协议)

| 并发 | before | **after** | Δ |
|---|---|---|---|
| C=1 | 28.13 ms / 32.67 t/s | **26.48 ms / 34.53 t/s** | −1.65 ms / +5.7% |
| C=2 | 30.55 / 59.89 | **28.81 / 63.19** | −1.74 ms / +5.5% |
| C=4 | 33.60 / 100.16 | **31.79 / 104.98** | −1.81 ms / +4.8% |
| C=4 / OUT=2048(长跑) | — | **29.56 ms / 128.15 t/s** | 8/8 完成,0 错误 |

* 每层 `compute`(服务内,**engine-only**):**0.304 → 0.239 ms(−21%)**;EP 0.011–0.023 ms。
* `rest` 稳态最小 **0.335 ms ≤ 0.40 ms** ✅。
* 数值门禁 `test_block23_equiv.py`: **`OK=7 BAD=1`,与基线逐字相同**。
* 稳定性:harness **16,000 次调用**(`SHARD_WD=60` 收紧)无 hang;服务端长跑 0 ERROR/WATCHDOG。

## 三、风险与回退

* 这段自旋是**第 184 轮为规避"丢票 ⇒ 300 s 看门狗 abort"加的安全措施**。
  降到 200 **削弱了保护强度**(根因窗口未被消除)。
* **一键回退**:`EXTRA_ENV="XIAOTU_MOE_PUBLISH_SETTLE=2000"`。
* **后续**:用"精确握手"(发布者只等"仍在上一代的 worker 数 == 0")替换有界自旋,
  既零成本又比 2000 更安全。

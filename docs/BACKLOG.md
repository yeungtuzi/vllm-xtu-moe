...
| 上游 PR(PR1/2/3) | 🟡 待重新定策 | 用户 2026-09-09 提问:通用化目标下是否重排(见 §3 D9) |
...
| ID | 待办 | 为什么 | 优先级 |
|---|---|---|---|
| **T20** | **重新定策 PR 集合**(见 §3 D9):PR1 是否扩到 fp8/int4 再提?PR3 是否先不提? | 通用化目标改变了 PR 的叙事与范围 | **P0(决策)** |
...
| **T23** | 上游漂移巡检:`bash scripts/check_upstream_drift.sh`,定期 rebase 三个分支 | 上游 ~41 commits/天 | P1 |
| **T24** | 若 T20 决定重排:同步更新 `patches/UPSTREAM_PRS.md` + 三个 `pr*_body.md` + 分支 | 保持快照与 GitHub 一致 | P1 |
...
| **D9** | **PR 集合是否因"通用化目标"重排 —— 待用户拍板(见下)** | 2026-09-09 | 用户提问 |
...
### D9 待决:PR 集合怎么重排?(用户 2026-09-09 提问)
...
| PR | 现状 | 建议 | 理由 |
|---|---|---|---|
| **PR1** `VLLM_EXPERTS_LOAD_DEVICE=cpu` (#56118) | draft,3 文件 +59/−2 | **扩到 fp8/int4 oracle 后再提** | 它是"混合模式"的总开关,通用化后 fp8 才是主用例;先提 mxfp4 再补 fp8 会显得零碎 |
| **PR2** fp8 o_proj SM8.x (#56119) | draft,4 文件 +422/−12 | **可以提** | 独立、明确的 bug 修复,不依赖通用化叙事 |
| **PR3** SM80 port (#56120) | draft,21 文件 +6350/−119 | **先不提,等通用性方案定了再决定拆分** | 最大、最 DS-V4 特定;若重心转向"多模型通用",它的性价比需要重新评估 |
| **新增** T21 两个上游 bug 修复 | 未创建 | **优先做** | 与通用化目标直接相关、体量小、对主线有普遍价值 |
| **RFC** | 未发 | 先发 layerwise prefill,再发"通用 CPU 后端 + 混合模式" | 混合模式需要上游设计层面的扩展点 |

**"不动 draft" 的代价**:分支会漂移(上游 ~41 commits/天),但 draft 本身不占资源;
我们有 `scripts/check_upstream_drift.sh` + 已演练过两次 rebase(其中 PR3 那次是结构性冲突,
手动重port 花了较多时间)。**结论:先不动是安全的;真正的成本在"PR3 每次结构性重构都要重做一遍"**,
这也是建议 PR3 暂缓的原因之一。
...
3. **T20/D9**:用户拍板 PR 集合,然后同步 `patches/UPSTREAM_PRS.md`。

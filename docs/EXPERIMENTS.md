...
| # | 实验 | 目的 | 判据 | 状态 |
|---|---|---|---|---|
| **G-4** | `/tmp/g4.sh`：**全程 profiler**（`CALLS=1000000`）跑 32K/MBT=16384 | 拿「所有核之和 vs 墙钟」的真实差额（判定那 19 s 是什么） | 叶子合计应接近 36.86 s;差额即真实未记录量 | **RUNNING** |

**动机**:B32 说「窗口不完整」,但 `CALLS=40` 在 1 chunk 下其实**接近完整**(42 次 MoE 调用)。
更可能是 **profiler 只在 `_maybe_profile()` 附近 ON**,窗口之外的核没被记录。
⇒ 把 `CALLS` 调到极大让全程采集,才能判定那 19 s 是「未记录的真实 GPU 工作」还是「真的不存在」。

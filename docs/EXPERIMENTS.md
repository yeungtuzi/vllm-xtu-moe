...
| # | 实验 | 目的 | 判据 | 状态 |
|---|---|---|---|---|
| **D-2** | `/tmp/d_v41.sh` **V4.1 @256K + MBT=4096**（KV=8031830016，TP=2） | **最高优先级**:修 LTS 模型长 prompt | `Successful=1` 且 `new_empty=0` | **RUNNING** |
| **D-1c** | GLM @256K + MBT=8192（**需重跑**） | 交付:GLM 256K | 同 D-1 | 待 D-2 |

**已失败的尝试（留档）:**

| 尝试 | 配置 | 结果 |
|---|---|---|
| D-0a | GLM 256K, MBT=16384, KV 5.62G | ❌ OOM（KV cap 给宽 0.86 GiB） |
| D-0b | GLM 256K, MBT=16384, KV 4.76G | ❌ 启动竞态（显存未释放） |
| D-1 | GLM 256K, MBT=16384, KV 4.76G | ❌ **OOM=5**：满卡 39.40/39.49 GiB，`Tried to allocate 120 MiB` ⇒ **降 MBT 是杠杆** |
| ~~D-1b~~ | GLM 256K, MBT=8192 | ⚠️ **被我自己杀掉** —— 我启动 V4.1 时没先停它，两者争 GPU，V4.1 的清场杀了 GLM 的服务。**需重跑（D-1c）。** |

**⚠️ 操作纪律（新增）**:同一时刻只能有一个交付实验在跑。
**启动新实验前必须先停掉正在跑的** —— 否则新脚本的清场会杀掉旧实验的服务。
（这正是本会话"前置核查不足"的又一次表现：我知道两者争 GPU，却没有先停旧的。）

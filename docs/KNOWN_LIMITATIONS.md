...
| **投机解码(MTP)默认开 `SPEC_K=1`,代价是 KV −27%** | 插件能正确识别并常驻 GLM 的第 45 层 draft(3.38 GiB/rank,CUDA graph 捕获正常)。接受长度 k=1 **1.46** / k=4 **1.63**;实测 **k=1 的 TPOT 约 −3%(45.7 → 44.2 ms,噪声内),但 KV 池 915,487 → 666,366(−27%,256K 并发 3.49× → 2.54×)**,且 ITL 脉冲化(46 → 64 ms);**k>1 明确更差**(k=4 掉到 11.2 tok/s)⇒ 默认 `SPEC_K=1`,`SPEC_K=0` 可关,**不要开 >1**。开 MTP 必须配 `GPU_UTIL=0.82` 或 `GP_PREFILL=0`(见 §8.2)。数据见 `MODEL_GUIDES.md` §2.6 |
...
### 10.1 GPU 预填充的真正瓶颈是 **DMA 带宽,不是转置**(2026-09-20 分层插桩)

`XIAOTU_GPF_STAGE=1` 在服务里实测 DSV4.1(qlen≈4,274):`dma=599 ms / asm=0 ms / tr=6.7 ms / 层`。

* **DMA 占 98.9%,K-major 转置只占 1.1%** ⇒ **"离线预转 K-major"收益上限只有 1%,不值得做**。
* **DMA 实测 ~6.4 GB/s,而本机 PCIe 线速 26.86 GB/s** ⇒ **只跑到 24%,有 ~4.2× 空间**
...
**做预填充测量必须换 prompt 或关前缀缓存**,否则会得到"预填充只要 0.87 s"的假象。

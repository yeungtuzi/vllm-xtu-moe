...
**工作区干净；`vllm-up-133b71e0b` 现在在分支 `xtu/glm53-sm80-pr4` @ `43aef91579`（已打 pr4，B91）。**
...
稀疏 MLA 核 **24.80 → 8.60 s(2.89×)**,其余各项不变。

**✅ 已打进生产树(B91,用户 2026-09-21 决定)**:`vllm-up-133b71e0b` 现在在分支
**`xtu/glm53-sm80-pr4` @ `c7d9f7b9c5`**(= pr4 提交 `43aef91579` **+ 回退我们的 MTP 补丁**,见 EXPERIMENTS B100)(不再 detached 于 `eddc6d0eb7`)。
**正确性已验**:核级 8/8 用例 OK 且 1-D 与 2-D 核的 `max_abs` **逐位相同**;端到端首 token 全一致、
跨臂差异小于同臂噪声。**性能本次没测**(用户要求 MiMo-2.6 适配后统一重测)。
...
>    打回去预期注意力 24.8→~8.6 s、TTFT 47.4→~30 s 量级。
>    **✅ 已于 2026-09-21 打进生产树(见上,B91);性能待 MiMo-2.6 适配后统一重测。**

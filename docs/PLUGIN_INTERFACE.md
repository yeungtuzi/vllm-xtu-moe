...
| C7 | `VLLM_XIAOTU_GPU_PREFILL_MIN_TOKENS` | 无(插件不设=0) | 长预填充场景设 **4096**(§603:实测盈亏平衡 ~2860 token,用 1024 会让 1-3K prompt 白付 ~9 s) | `_gp_min > 0` 是 GPU 预填充分支的**唯一**开关;不设 ⇒ 非常驻层永远走 CPU | §340 |

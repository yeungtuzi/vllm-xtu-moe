...
### 3.1 ⭐ 收益由 **chunk 大小**决定(机制与实测)

**机制**:每个 chunk 都要把**该步所有层的专家权重整份 H2D 搬一遍**,与这个 chunk 里有多少 token
**无关** ⇒ **单 chunk 成本近似固定**(`≈8.9 s 固定 + 0.79 ms/token`)。因此收益 = 这个固定成本
能被多大的 chunk 摊薄。

**GLM-5.3-Flash(FP8)—— chunk 被 KDA 钉死,所以只有 1.2×**(同一 prompt,切不同 chunk)

| chunk | 纯 CPU | GPU 流式 | 收益 |
|---|---|---|---|
| **2176**(GLM 的实际上限:`block_size=2176`) | 11.3 s | 9.7 s | 1.2×(省 1.6 s) |
| 4096(反事实:去掉 KDA 约束) | 21.3 s | 9.7 s | **2.2×** |
| 8192(反事实) | 42.6 s | 9.7 s | **4.4×** |

⇒ **GPU 列三种 chunk 下都是 9.7 s** = 固定成本的直接证据;CPU 列随 chunk 线性涨。
**GLM 的 1.2× 是"被 2176 封顶",不是这条路没用。**

**每层成本分解(GLM,实测)**:`装配 207 ms`(权重 H2D 3.62 GB/rank)+ `GPU MoE 23 ms`
⇒ **装配占 90%**。隔离环境实测装配 **134.9 ms = 26.86 GB/s**,即本机 H2D 天花板
(1-D 与 pitched 2-D 同速,锁页/NUMA 交错/两 rank 并发都不改变);服务里的 207 ms
是与 vLLM 自身 PCIe 流量(TP=2 all-reduce,GPU0/1 无 NVLink)争用的结果。
⇒ 优化点是**把装配与 attention 重叠**(默认开的 side stream),不是"搬得更快"。

**DeepSeek-V4.1-Flash —— 能吃满 chunk,所以是另一个量级**(同一 **13.8K** prompt)

| 版本 | TTFT | prefill |
|---|---|---|
| 修复前 | 76.5 s | 181 tok/s |
| **修复后**(环形槽复用 + 设备级判定 + 快速转置) | **8.008 s** | **1725 tok/s** |
| 反向验证(KV 只留 1 GiB,泄漏仍在) | 18.35 s | 872 tok/s |

⇒ **9.6×**;对照 CPU 预填充(13.8K × 3.9 ms ≈ 54 s)是 **6.7×**。
三个修复都在当前插件里(`gpu_prefill.py` 的 `_reuse_moe` / 环形槽 / `fits_device`)。

**GLM 侧还有一个坑**:chunk 被 `block_size=2176` 钉死 ⇒ 插件默认阈值 4096 **永远不会触发**;
...
⇒ **TTFT ≈ 0.84 s 固定 + 11.4 ms/token**(线性),所以**短 prompt 的"prefill tok/s"会被固定开销压低**
(256 token 只有 68,要到 4K 才收敛到 86)—— 读这个指标必须按长 prompt。
该序列是**关掉 GPU 流式预填充**时的 CPU 专家路径;打开后见下表。

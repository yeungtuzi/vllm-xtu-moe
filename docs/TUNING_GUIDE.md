...
## 8. GLM-5.3-Flash @ 256K：MBT 三档的收益 / 代价对照（2026-09-21 实测）

> 场景：TP=2、`MAXLEN=262144`、`KV_CACHE_BYTES=5113807360`（4.76 GiB，引擎反算真实需求，**不加乘性余量**）、
> 单条 16384-token prompt、C=1。**全部为零代码改动，仅调 `MBT`。**

| MBT | chunks | prefill | TTFT | 上下文上限（⚠️推断） | 依据 |
|---|---|---|---|---|---|
| **4096** | 4 | **154.5 tok/s** | ~106 s | **≈ 730K–785K** | prefill 为 B2 实测；上限为推算 |
| **8192** | 2 | **233.6 tok/s** | 70.2 s | **≈ 496K–551K** | 256K 实测 |
| **12288** | 2 | **266.3 tok/s** | 61.6 s | ≈ 262K–496K | 256K 实测（判据 `[fp8-asm]=168`） |
| ~~13312~~ | 2 | ❌ OOM | — | — | 实测：第 41 层 OOM |
| ~~14336~~ | 2 | ❌ OOM | — | — | 实测：第 41 层 OOM |
| ~~16384~~ | 1 | ❌ OOM | — | — | 实测：第 35 层 OOM |

### 为什么是这三档在权衡

**根因（实测）：KDA 线性注意力的 chunk 状态 `h` 逐层累积，且 `h ∝ NT = T/BT`。**
一次 256K 请求内设备显存**单调爬升 21.65 GiB**（外部轮询 `nvidia-smi`），≈0.62 GiB/层，
与 `h` 的每层占用 × **34 个 KDA 层**吻合。

| MBT | NT | 单层 h | 34 层累积 |
|---|---|---|---|
| 4096 | 64 | 0.125 GiB | **4.25 GiB** |
| 8192 | 128 | 0.250 GiB | **8.50 GiB** |
| 12288 | 192 | 0.375 GiB | **12.75 GiB** |
| 16384 | 256 | 0.500 GiB | 17.00 GiB（**放不下**） |

**⇒ `MBT` 越小 ⇒ `h` 越小 ⇒ 越多显存可留给 KV ⇒ 上下文越长；但 chunk 数越多 ⇒ 权重重流越多 ⇒ 越慢。**
**每 chunk 都要重流全部 141.8 GiB 专家权重** —— 这是吞吐代价的来源：

```
MBT=4096 → 4 chunks ⇒ H2D 567 GiB ⇒ 22.7 s
MBT=8192 → 2 chunks ⇒ H2D 284 GiB ⇒ 11.3 s
MBT=12288 → 2 chunks ⇒ H2D 284 GiB ⇒ 11.3 s   ← 与 8192 同 chunk 数,但首块更大 ⇒ 更快
```
...
| 若你的首要目标是… | 选 |
|---|---|
| **吞吐**（上下文 ≤ 25 万足够） | `MBT=12288`（266.3 tok/s） |
| **平衡**（约 50 万上下文） | `MBT=8192`（233.6 tok/s） |
| **最长上下文**（约 75 万） | `MBT=4096`（154.5 tok/s，慢 34%） |

**⚠️ 上下文上限那一列是**推断**，不是实测**：它由「`MBT=12288` 比 8192 多要 4.25 GiB 仍成功、
`13312` 多要 5.25 GiB 却失败」夹出激活侧余量，再折算成 KV。
**两条不确定性**：① 激活侧余量未必能等量转成 KV（定容时机不同）；
② 早期「1M 会禁用 GPU 预填充」的结论是另一套配置下测的，不能直接套用。
**⇒ 若要某个具体长度，应按该长度实测一次，而不是信这张表的上限列。**

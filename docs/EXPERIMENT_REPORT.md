...
>
> **对 CPU MoE 引擎的含义(实测,不是推断)**:引擎每层只读到 **3.2 GB/s**
> (256 专家 × 12 MB / 0.96 s),远低于 740 GB/s 的上限,所以它**不是带宽受限**;
> 它跑在 **0.64 TFLOP/s ≈ AVX-512 BF16 峰值的 6%**,瓶颈是 fp4→bf16 反量化的 ALU 工作。
> 第 1 版"CPU MoE 是带宽敏感型"的说法**已被本实测推翻**。
...
- **2026-09-09(第 4 版)** — ①§1.1 补内存规格 **DDR5-4800**(24×64 GiB,理论 921.6 GB/s);
  ...
  第 3 版"CPU MoE 是带宽敏感型"的说法(实测引擎只用到 3.2 GB/s、0.64 TFLOP/s,
  是反量化 ALU 受限)。依据:`report/stream_nt.c`、`report/rd1.c`、`/tmp/s_*`、

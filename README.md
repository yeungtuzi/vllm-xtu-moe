...
* 口径：`prefill (tok/s) = prompt_tokens / TTFT`、`decode (tok/s) = 1000 / TPOT`（均由同一次 `vllm bench serve` 换算）。
  > ⚠️ **decode 取 `median` TPOT**：CED 开臂的 `mean` TPOT 会被极少数（p99≈185 ms）离群解码步拉高，
  > 照抄 `mean` 会得出「CED 让 decode 慢 26%」的**假结论**，而 median TPOT 两臂完全相同（51.91 vs 51.84 ms）。
...
> † **长/C=2 两格原为「待测」，现已取得**（V4.1）；GLM 的长/C=2 仍待测。各格的配置与判据如下。
...
* **长 prompt 的实测判据**：
  ...
  * DeepSeek-V4.1-Flash：TTFT **18,097 ms**（median，C=1，CED 开）；`DISABLED=0`、`aten::new_empty=0`、`illegal=0`、`OOM=0`。
* **MBT 是 256K 下的关键旋钮**：GLM 在 `MBT=16384` 时 OOM（实测第 35 层），**`MBT=12288` 成功**（2 chunks，首块更大 ⇒ 效率更高）；
  ...
  （这一段是**开 CED 之前**的历史口径；开 CED 后同配置的 CED 关基线已是 434.5 tok/s，见下。）
* ⚠️ **短 prompt 三行的数值来自早期一轮 32K 预算的测量**（配置与上表所列的 256K 配置不同），
  **尚未在 256K 配置下重测**。
* **V4.1 的长两行已改用 CED（decoder-side SWA bounded replay）开的结果**，实测树为上游 PR
  [#56752](https://github.com/vllm-project/vllm/pull/56752) 的 `ced/pr56752`（= 本仓生产树 +3 commits）。
  同一批的**配对 A/B**（两臂同 seed、同配置，仅 `--no-swa-bounded-replay` 不同）：

  | 臂 | 长/C=1 prefill | 长/C=2 prefill | 长/C=1 decode | 长/C=2 decode | 失败 |
  |---|---|---|---|---|---|
  | CED 关（基线） | 434.5 tok/s | 334.6 tok/s | 19.3 tok/s | 3.9 tok/s | 0/16 |
  | **CED 开** | **903.9** | **602.1** | **19.3** | **7.7** | 0/16 |

  ⇒ **prefill 2.08×（C=1）/ 1.80×（C=2）**；decode 在 C=1 持平、C=2 快 **1.99×**；**零失败**（含两路并发）。
  同一基线（CED 关）在本题材下已是 **434.5 tok/s**，故表中旧值 355.0 亦被替换。
  细节、判据与**等价性口径**见 `docs/EXPERIMENTS.md` B84/B85（首 token 等价；「逐字节全文」在 ≥5K 上
  同臂就不可复现，不能用作判据）。

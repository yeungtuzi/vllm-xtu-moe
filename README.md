...
|---|---|---|---|---|---|
| DeepSeek-V4.1-Flash | 关 | 1 | — | — | 输出吞吐 **13.79** |
| (TP=2 · 1M · GPU 预填充 4096) | **开**(dspark k=5) | 1 | — | **34.0** | 输出吞吐 **18.38(+33%)** |
| | 关 | 4 | — | — | 输出吞吐 **41.62** |
...
> ⚠️ **prefill 列留空是刻意的**:ShareGPT 的 prompt 非定长,而这批数据**没有记录 mean prompt
> 长度** ⇒ prefill 不可换算。硬凑一个数就是编数据。要补得重跑并记下 prompt 长度。

...
| | 长 | 16,396 | 2 | 待测 † | 待测 † |
| **DeepSeek-V4.1-Flash**<br>TP=2 · util 0.85 · **MAXLEN 262144**<br>MBT 4096 · 投机解码关 | 短 | 128 | 1 | **254.3** | **19.8** |
| | 短 | 128 | 2 | **175.2** | 14.0 |
| | 长 | 16,384 | 1 | **115.8** | **18.7** |
| | 长 | 16,384 | 2 | 待测 † | 待测 † |
...
* **MBT 是 256K 下的关键旋钮**：GLM 在 `MBT=16384` 时 OOM（实测第 35 层），**`MBT=12288` 成功**（2 chunks，首块更大 ⇒ 效率更高）；
  ...
  三档的收益/代价对照见 `docs/TUNING_GUIDE.md` §8。V4.1 的 `MBT=16384` 会撞上装配期
  `aten::new_empty` 分配失败，`MBT=4096` 成功。

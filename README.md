...
2. **合批能再抬一个数量级**:同一长 prompt,C=1 → C=4 时
   **MiMo 179 → 1,845**(+10×)、**DeepSeek-V4.1 330 → 2,595**(+7.9×)——
   4 路并发把近 2 万 token 合成大批,同一份权重搬运费摊到 4 倍的 token 上。
   **这也解释了为什么短 prompt 看着"慢"**:84-153 tok 的活儿,固定开销占了大头。

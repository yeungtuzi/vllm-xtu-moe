...
| **DeepSeek-V4.1-Flash**<br>TP=2 · GPU 预填充<br>KV 封顶 1.0 GiB · MBT 16384 · 投机解码关 | 短 | 128 | 1 | **254.3** | **19.8** |
| | 短 | 128 | 2 | **175.2** | 14.0 |

> **MiMo-V2.5 仍受支持**（见「支持的模型」），但因其后继版本 MiMo-2.6 即将发布，本表**不再列出其性能数据**。

## 长上下文（256K）long prefill

上面那张表用的是 32K 上限下的预算；下表是**在必须保证 256K 上下文**（`MAXLEN=262144`）
约束下把其余显存让给 GPU 预填充的成绩。**单条 16384-token random prompt，C=1，前缀缓存开。**
...
* **MiMo-V2.5 无法在 256K 下运行**：其 KV 约 248 KiB/token（GLM 仅 19 KiB），256K 需 **61.9 GiB/rank**，
  超过单卡 35.5 GiB 预算 1.74 倍 —— 这是架构决定的，非配置问题。

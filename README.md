...
### ⚠️ 与 `lvllm + lk-moe` 的**等效参数同机对比**(2026-09-18;**我们更慢,如实公布**)

为回答"为什么比参考实现慢这么多",做了**可归因**的 A/B:两个 arm 跑在**同一个 conda env**
(vLLM 2.5.0 基座)里,参数照抄 `Lvllm/commands/dsv41_serve_tp2_3090_dspark.sh`,
CPU 线程数两边同为 60,GPU 预填充两边都关,客户端 `vllm bench serve` 且两个 arm 拿到
**逐字节相同**的 prompt。纯度断言:arm A 日志无 `vllm-xtu-moe`,arm B 有。
完整表与复现见 **[`report/tuning/AB_LVLLM_VS_XIAOTU.md`](report/tuning/AB_LVLLM_VS_XIAOTU.md)**。

| prompt | output | lk-moe `output_throughput` | **本插件** | B/A | lk-moe TTFT | **本插件 TTFT** | B/A | lk-moe TPOT | **本插件 TPOT** | B/A |
|---|---|---|---|---|---|---|---|---|---|---|
...
**速率**:prefill lk-moe **119-128 tok/s** vs 本插件 **85-95 tok/s**;解码 `1/TPOT`
lk-moe **43.9-44.3 tok/s** vs 本插件 **26.7-31.7 tok/s**。
⇒ **prefill 慢 1.34-1.40×,解码慢 1.4-1.6×。**

**差距的分解(不是单一原因)**
* **引擎层面一直慢 1.1-1.3×**:我们自己的引擎微基准门禁(同权重/同形状/同线程数,
  直接加载两个 `.so`)长期报"同日 lk 比值 **1.17 / 1.23**"(DEDUP=12 / 23);
  历史起点更差(2026-09-11 是 **2.14×**)。这是**已知的、在持续收敛的**差距。
* **设计层面的结构性根因**(门禁 docstring 已记录):**我们的 GEMM 沿 `K` 维向量化,
  lk_moe 沿"输出列"向量化**。解码 M=1 时,按输出列切分能让每个线程独立负责若干输出列、
  权重每字节只用一次;按 K 切分则更依赖跨线程归约、复用更差。这是解码差距的主要来源。
* **配置/集成还有约 1.2-1.3×**:正在做旋钮扫描(`SPIN_IDLE_US` / `THREADS` / `NSLICE_SMALL` /
  `ASYNC`);已排除 `SPIN_IDLE_US=0`(实测更差:TPOT 53.6/56.5 ms vs 基线 37.4/33.3 ms)。

**结论**:把本插件当作"**在无 AMX 的 x86 上、用 GPU 显存换专家权重的可用方案**"是成立的
(功能、1M 上下文、投机、数值门禁、确定性都验过),但**在 CPU MoE 引擎的每层效率上目前落后
参考实现 1.1-1.6×**,且差距**两个维度同时存在**。这一条不藏着,写在这里。

...
| **MiMo-V2.6-Flash-RL**<br>TP=2 · util 0.85 · **MAXLEN 131072**<br>MBT 8192 · **MTP k=1** · **GPU 预填开(门槛 4096)** · 含形状预热 | 短 | 128 | 1 | **236.3** | **31.8** |
| | 短 | 128 | 2 | 54.4 ⚠️ | **59.6** |
| | 长 | 16,384 | 1 | **745.6** | **27.5** |
| | 长 | 16,384 | 2 | **738.2** | **55.1** |
...
> ⚠️ **短上下文 C=2 的 prefill 偏低是「并发 prefill 不重叠」造成的实测现象**(见 `docs/EXPERIMENTS.md` B123):128-token 的 prompt 在 C=2 下第二个请求要等第一个跑完,于是 TTFT ≈ 对方的 prefill+decode(随输出长度线性增长:out=8 → 0.89 s,out=128 → 4.2 s);**decode 侧仍会批处理**(长 C=2 的 decode 聚合正好翻倍)。直连 HTTP 实测单请求仅 0.46–1.57 s,说明**引擎本身不慢,是 prefill 不并发**。其余格子均为同一批实测。
